...
function renderReferencePreviews() {
  ...
  selectedReferences.forEach((files, index) => {
    ...
    item.setAttribute('aria-label', `${referenceRoles[index]}参考图上传区，${files.length} / ${maxReferencesPerRole}`);
    let dragDepth = 0;
    item.addEventListener('dragenter', (event) => {
      if (!event.dataTransfer?.types?.includes('Files')) return;
      event.preventDefault(); dragDepth += 1; item.classList.add('dragging');
    });
    ...
    item.addEventListener('dragleave', (event) => {
      ...
      if (!dragDepth) item.classList.remove('dragging');
    });
    item.addEventListener('drop', (event) => {
      if (!event.dataTransfer?.files?.length) return;
      event.preventDefault(); dragDepth = 0; item.classList.remove('dragging');
      addReferencesToRole(index, [...event.dataTransfer.files]);
    });
    const heading = document.createElement('div');
    heading.className = 'referencePreviewHeading';
    const indexBadge = document.createElement('span');
    indexBadge.className = 'referenceIndex';
    indexBadge.textContent = `0${index + 1}`;
    const headingCopy = document.createElement('span');
    headingCopy.className = 'referenceHeadingCopy';
    const title = document.createElement('strong');
    title.textContent = referenceRoles[index];
    const subtitle = document.createElement('small');
    subtitle.textContent = index === 0 ? '锁定产品身份、结构与材质细节' : index === 1 ? '约束姿态、手部接触与真实受力' : '参考镜头、光线、色调与整体风格';
    headingCopy.append(title, subtitle);
    const required = document.createElement('span');
    required.className = `referenceRequired${index === 0 ? ' is-required' : ''}`;
    required.textContent = index === 0 ? '必填' : '可选';
    const counter = document.createElement('span');
    counter.className = 'referenceSlotCount';
    counter.textContent = `${files.length} / ${maxReferencesPerRole}`;
    heading.append(indexBadge, headingCopy, required, counter);
    item.append(heading);
    ...
    media.setAttribute('role', 'list');
    files.forEach((file, fileIndex) => {
      const imageWrap = document.createElement('div'); imageWrap.className = 'referenceThumb'; imageWrap.setAttribute('role', 'listitem');
      imageWrap.title = file.name || `${referenceRoles[index]}参考图 ${fileIndex + 1}`;
      ...
      fileLabel.className = 'referenceFileName';
      fileLabel.textContent = file.name || `参考图 ${fileIndex + 1}`;
      fileLabel.title = file.name || '';
      imageWrap.append(image, fileLabel, remove); media.append(imageWrap);
    });
    ...
    add.setAttribute('aria-label', atLimit ? `${referenceRoles[index]}已达到 ${maxReferencesPerRole} 张上限` : `向${referenceRoles[index]}添加参考图`);
    add.innerHTML = `<span class="uploadGlyph" aria-hidden="true">${atLimit ? '✓' : '+'}</span><span class="referenceAddCopy"><strong>${atLimit ? '已达上传上限' : files.length ? '继续添加' : '拖放图片或点击选择'}</strong><small>${atLimit ? `最多 ${maxReferencesPerRole} 张` : `还可添加 ${maxReferencesPerRole - files.length} 张 · JPG / PNG / WebP / MPO`}</small></span>`;
